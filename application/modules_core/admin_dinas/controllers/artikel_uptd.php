...
class artikel_uptd extends MX_Controller {
	...
	public function index($uri=0)
	{
		if($this->session->userdata("logged_in")!="" && $this->session->userdata("tipe_user")=="admin_dinas")
		{
			...
			$d['dt_data_artikel_uptd'] = $this->app_global_admin_dinas_model->generate_index_artikel_uptd($this->config->item("limit_item_big"),$uri,$filter);
			
			$this->load->view($_SESSION['site_theme'].'/bg_header',$d);
			if($_SESSION['site_slider_always']=="yes"){$this->load->view($_SESSION['site_theme'].'/bg_slider');}
			$this->load->view($_SESSION['site_theme'].'/bg_breadcumb');
			$this->load->view($_SESSION['site_theme'].'/admin_dinas/artikel_uptd/bg_home');
	...
	public function edit($id_param)
	{
		if($this->session->userdata("logged_in")!="" && $this->session->userdata("tipe_user")=="admin_dinas")
		{
			$this->breadcrumb->append_crumb('Beranda', base_url());
			$this->breadcrumb->append_crumb('Dashboard', base_url().'admin_dinas/dashboard');
			$this->breadcrumb->append_crumb("Artikel UPTD", base_url().'admin_dinas/artikel_uptd');
			...
			if($_SESSION['site_slider_always']=="yes"){$this->load->view($_SESSION['site_theme'].'/bg_slider');}
			$this->load->view($_SESSION['site_theme'].'/bg_breadcumb');
			$this->load->view($_SESSION['site_theme'].'/admin_dinas/artikel_uptd/bg_input');
			$this->load->view($_SESSION['site_theme'].'/bg_footer');
		}
		else
		{
		...
		if($this->session->userdata("logged_in")!="" && $this->session->userdata("tipe_user")=="admin_dinas")
		{
			...
			if($tipe=="edit")
			{
				if(empty($_FILES['userfile']['name']))
				{
					...
					redirect("admin_dinas/artikel_uptd");
				}
				else
				{
	...
	public function approve($id_param,$up)
	{
		if($this->session->userdata("logged_in")!="" && $this->session->userdata("tipe_user")=="admin_dinas")
		{
			$where['id_uptd_artikel'] = $id_param;
			$upd['stts'] = $up;
			$this->db->update("dlmbg_uptd_artikel",$upd,$where);
			redirect("admin_dinas/artikel_uptd");
		}
		else
		{
