...
class artikel extends MX_Controller {
	...
	public function index($uri=0)
	{
		if($this->session->userdata("logged_in")!="" && $this->session->userdata("tipe_user")=="pengawas")
		{
			...
			$d['dt_data_artikel'] = $this->app_global_pengawas_model->generate_index_artikel($this->config->item("limit_item_big"),$uri,$filter);
			
			$this->load->view($_SESSION['site_theme'].'/bg_header',$d);
			if($_SESSION['site_slider_always']=="yes"){$this->load->view($_SESSION['site_theme'].'/bg_slider');}
			$this->load->view($_SESSION['site_theme'].'/bg_breadcumb');
			$this->load->view($_SESSION['site_theme'].'/pengawas/artikel/bg_home');
	...
	public function tambah()
	{
		if($this->session->userdata("logged_in")!="" && $this->session->userdata("tipe_user")=="pengawas")
		{
			$this->breadcrumb->append_crumb('Beranda', base_url());
			$this->breadcrumb->append_crumb('Dashboard', base_url().'pengawas/dashboard');
			$this->breadcrumb->append_crumb("Artikel Pengawas", base_url().'pengawas/artikel');
			...
			if($_SESSION['site_slider_always']=="yes"){$this->load->view($_SESSION['site_theme'].'/bg_slider');}
			$this->load->view($_SESSION['site_theme'].'/bg_breadcumb');
			$this->load->view($_SESSION['site_theme'].'/pengawas/artikel/bg_input');
			$this->load->view($_SESSION['site_theme'].'/bg_footer');
		}
		else
		{
	...
	public function edit($id_param)
	{
		if($this->session->userdata("logged_in")!="" && $this->session->userdata("tipe_user")=="pengawas")
		{
			$this->breadcrumb->append_crumb('Beranda', base_url());
			$this->breadcrumb->append_crumb('Dashboard', base_url().'pengawas/dashboard');
			$this->breadcrumb->append_crumb("Artikel UPTD", base_url().'pengawas/artikel');
	...
	public function simpan()
	{
		if($this->session->userdata("logged_in")!="" && $this->session->userdata("tipe_user")=="pengawas")
		{
			...
			$id['id_pengawas_artikel'] = $this->input->post("id_param");
			...
			if($tipe=="tambah")
			{
				...
				{
					$config['upload_path'] = './asset/images/artikel-pengawas/';
					...
					if ($this->upload->do_upload("userfile")) {
						...
						/* PATH */
						$source             = "./asset/images/artikel-pengawas/".$data['file_name'] ;
						$destination_thumb	= "./asset/images/artikel-pengawas/thumb/" ;			 
						...
						$in_data['id_super_unit_kerja'] = $this->session->userdata("id_unit_kerja");
						$in_data['tanggal'] = time()+3600*7;
						$in_data['stts'] = "0";
						$this->db->insert("dlmbg_pengawas_artikel",$in_data);
						unlink($source);
						
						redirect("pengawas/artikel");
			...
			else if($tipe=="edit")
			{
				if(empty($_FILES['userfile']['name']))
				{
					$in_data['judul'] = $this->input->post("judul");
					$in_data['isi'] = $this->input->post("isi");
					$this->db->update("dlmbg_pengawas_artikel",$in_data,$id);
					redirect("pengawas/artikel");
				}
				else
				{
					...
					if ($this->upload->do_upload("userfile")) {
						...
						$old_thumb	= "./asset/images/artikel-pengawas/thumb/".$this->input->post("gambar")."" ;
						unlink($source);
						unlink($old_thumb);
