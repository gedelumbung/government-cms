...
class galeri_uptd extends MX_Controller {
	...
	public function index($uri=0)
	{
		if($this->session->userdata("logged_in")!="" && $this->session->userdata("tipe_user")=="uptd")
		{
			...
			$d['dt_galeri_uptd'] = $this->app_global_uptd_model->generate_index_galeri_uptd($this->config->item("limit_item_big"),$uri,$filter);
			
			$this->load->view($_SESSION['site_theme'].'/bg_header',$d);
			if($_SESSION['site_slider_always']=="yes"){$this->load->view($_SESSION['site_theme'].'/bg_slider');}
			$this->load->view($_SESSION['site_theme'].'/bg_breadcumb');
			$this->load->view($_SESSION['site_theme'].'/uptd/galeri_uptd/bg_home');
	...
	public function tambah()
	{
		if($this->session->userdata("logged_in")!="" && $this->session->userdata("tipe_user")=="uptd")
		{
			$this->breadcrumb->append_crumb('Beranda', base_url());
			$this->breadcrumb->append_crumb('Dashboard', base_url().'uptd/dashboard');
			$this->breadcrumb->append_crumb("Galeri UPTD", base_url().'uptd/galeri_uptd');
			...
			if($_SESSION['site_slider_always']=="yes"){$this->load->view($_SESSION['site_theme'].'/bg_slider');}
			$this->load->view($_SESSION['site_theme'].'/bg_breadcumb');
			$this->load->view($_SESSION['site_theme'].'/uptd/galeri_uptd/bg_input');
			$this->load->view($_SESSION['site_theme'].'/bg_footer');
		}
		else
		{
	...
	public function edit($id_param)
	{
		if($this->session->userdata("logged_in")!="" && $this->session->userdata("tipe_user")=="uptd")
		{
			$this->breadcrumb->append_crumb('Beranda', base_url());
			$this->breadcrumb->append_crumb('Dashboard', base_url().'uptd/dashboard');
			$this->breadcrumb->append_crumb("List Download", base_url().'uptd/galeri_uptd');
			...
			$get = $this->db->get_where("dlmbg_uptd_galeri_uptd",$where)->row();
			
			$d['judul'] = $get->judul;
			$d['gambar'] = $get->gambar;
			
			$d['id_param'] = $get->id_uptd_galeri_uptd;
			$d['tipe'] = "edit";
	...
	public function simpan()
	{
		if($this->session->userdata("logged_in")!="" && $this->session->userdata("tipe_user")=="uptd")
		{
			...
			$id['id_uptd_galeri_uptd'] = $this->input->post("id_param");
			...
			if($tipe=="tambah")
			{
				...
				{
					$config['upload_path'] = './asset/images/galeri-uptd/';
					...
					if ($this->upload->do_upload("userfile")) {
						...
						/* PATH */
						$source             = "./asset/images/galeri-uptd/".$data['file_name'] ;
						$destination_thumb	= "./asset/images/galeri-uptd/thumb/" ;
						$destination_medium	= "./asset/images/galeri-uptd/medium/" ;
						...
						$in_data['stts'] = "0";
						$this->db->insert("dlmbg_uptd_galeri_uptd",$in_data);
				
						unlink($source);
						
						redirect("uptd/galeri_uptd");
			...
			else if($tipe=="edit")
			{
				...
				{
					$in_data['judul'] = $this->input->post("judul");
					$this->db->update("dlmbg_uptd_galeri_uptd",$in_data,$id);
					redirect("uptd/galeri_uptd");
				}
				else
				{
					...
					if ($this->upload->do_upload("userfile")) {
						...
						$old_thumb	= "./asset/images/galeri-uptd/thumb/".$this->input->post("gambar")."" ;
						$old_medium	= "./asset/images/galeri-uptd/medium/".$this->input->post("gambar")."" ;
						unlink($source);
						unlink($old_thumb);
						unlink($old_medium);
