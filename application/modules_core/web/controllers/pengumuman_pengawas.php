...
class pengumuman_pengawas extends MX_Controller {
   ...
   public function index($uri=0)
   {
      ...
      $this->breadcrumb->append_crumb('Indexs Pengumuman Pengawas', '/');
      ...
      $d['dt_index_pengumuman'] = $this->app_global_model->generate_index_pengumuman_pengawas($this->config->item('limit_item_medium'),$uri,$filter);
      ...
      $this->load->view($_SESSION['site_theme'].'/pengumuman_pengawas/bg_home');
   ...
   {
      $where['id_pengawas_pengumuman'] = $id_pengumuman;
      $get_data = $this->db->get_where("dlmbg_pengawas_pengumuman",$where);
      $ret_data = $get_data->row();
      if($get_data->num_rows()>0)
      {
			$this->breadcrumb->append_crumb('Beranda', base_url());
			$this->breadcrumb->append_crumb('Indexs Pengumuman Pengawas', base_url().'web/pengumuman_pengawas');
			...
			$d['dt_detail_pengumuman'] = $this->app_global_model->generate_detail_pengumuman_pengawas($id_pengumuman,$tipe);

			$this->load->view($_SESSION['site_theme'].'/bg_header',$d);
			if($_SESSION['site_slider_always']=="yes"){$this->load->view($_SESSION['site_theme'].'/bg_slider');}
			$this->load->view($_SESSION['site_theme'].'/bg_breadcumb');
			$this->load->view($_SESSION['site_theme'].'/bg_left');
			$this->load->view($_SESSION['site_theme'].'/pengumuman_pengawas/bg_detail');
			$this->load->view($_SESSION['site_theme'].'/bg_right');
			$this->load->view($_SESSION['site_theme'].'/bg_footer');
      }
      else
