...
class berita_pengawas extends MX_Controller {
   ...
   public function index($uri=0)
   {
      ...
      $this->breadcrumb->append_crumb('Indexs Berita Pengawas', '/');
      ...
      $d['dt_index_berita'] = $this->app_global_model->generate_index_berita_pengawas($_SESSION['site_limit_berita_slider'],$uri,$filter);
      ...
      $this->load->view($_SESSION['site_theme'].'/berita_pengawas/bg_home');
   ...
   {
      $where['id_pengawas_berita'] = $id_berita;
      $get_data = $this->db->get_where("dlmbg_pengawas_berita",$where);
      $ret_data = $get_data->row();
      if($get_data->num_rows()>0)
      {
			$this->breadcrumb->append_crumb('Beranda', base_url());
			$this->breadcrumb->append_crumb('Indexs Berita Pengawas', base_url().'web/berita_pengawas');
			...
			$d['dt_detail_berita'] = $this->app_global_model->generate_detail_berita_pengawas($id_berita);

			$this->load->view($_SESSION['site_theme'].'/bg_header',$d);
			if($_SESSION['site_slider_always']=="yes"){$this->load->view($_SESSION['site_theme'].'/bg_slider');}
			$this->load->view($_SESSION['site_theme'].'/bg_breadcumb');
			$this->load->view($_SESSION['site_theme'].'/bg_left');
			$this->load->view($_SESSION['site_theme'].'/berita_pengawas/bg_detail');
			$this->load->view($_SESSION['site_theme'].'/bg_right');
			$this->load->view($_SESSION['site_theme'].'/bg_footer');
      }
      else
