...
class galeri_uptd extends MX_Controller {
   ...
   {  
      ...
      $d['dt_index_data'] = $this->app_global_model->generate_index_kecamatan();
      ...
      $this->load->view($_SESSION['site_theme'].'/galeri_uptd/bg_home');
      $this->load->view($_SESSION['site_theme'].'/bg_right');
      $this->load->view($_SESSION['site_theme'].'/bg_footer');
   }
 
   public function kecamatan($id_param,$url,$uri=0)
   {  
      $d['menu_atas'] = $this->app_global_model->generate_menu('0','atas',$h='','treemenu1');
      $d['menu_bawah'] = $this->app_global_model->generate_menu('0','bawah',$h='');
      $d['dt_bidang'] = $this->app_global_model->generate_menu_bidang();
      ...
      $this->breadcrumb->append_crumb('Beranda', base_url());
	  $this->breadcrumb->append_crumb('Galeri UPTD', base_url().'web/galeri/');
	  $this->breadcrumb->append_crumb($get->kecamatan, '/');
      $d['dt_index_data'] = $this->app_global_model->generate_index_galeri_uptd($id_param,$this->config->item("limit_item_super_big"),$uri);
