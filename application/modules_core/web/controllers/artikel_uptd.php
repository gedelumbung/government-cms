...
class artikel_uptd extends MX_Controller {
   ...
   {  
	  ...
	  $d['dt_kecamatan'] = $this->db->get("dlmbg_super_kecamatan");
	
      $this->breadcrumb->append_crumb('Beranda', base_url());
	  $this->breadcrumb->append_crumb('Indexs Artikel UPTD', '/');

      $d['dt_index_artikel_uptd'] = $this->app_global_model->generate_index_artikel_uptd($this->config->item("limit_item"),$uri);
      ...
      $this->load->view($_SESSION['site_theme'].'/artikel_uptd/bg_home');
   ...
   {
		$where['id_uptd_artikel'] = $id_artikel;
		$get_data = $this->db->get_where("dlmbg_uptd_artikel",$where);
		$ret_data = $get_data->row();
	  
		$this->breadcrumb->append_crumb('Beranda', base_url());
		$this->breadcrumb->append_crumb('Indexs Artikel Uptd', base_url().'web/artikel_uptd');
	  ...
	  $d['dt_agenda_pengawas'] = $this->app_global_model->generate_menu_agenda_pengawas($_SESSION['limit_footer_artikel_sekolah'],0);
		...
		$d['dt_detail_artikel_uptd'] = $this->app_global_model->generate_detail_artikel_uptd($id_artikel);
		
		$this->load->view($_SESSION['site_theme'].'/bg_header',$d);
		if($_SESSION['site_slider_always']=="yes"){$this->load->view($_SESSION['site_theme'].'/bg_slider');}
		$this->load->view($_SESSION['site_theme'].'/bg_breadcumb');
		$this->load->view($_SESSION['site_theme'].'/bg_left');
		$this->load->view($_SESSION['site_theme'].'/artikel_uptd/bg_detail');
		$this->load->view($_SESSION['site_theme'].'/bg_right');
		$this->load->view($_SESSION['site_theme'].'/bg_footer');
