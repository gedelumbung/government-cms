...
class agenda_dinas extends MX_Controller {
   ...
   {  
      ...
      $d['dt_index_agenda'] = $this->app_global_model->generate_index_agenda_dinas($id_param,$this->config->item("limit_item_medium"),$uri);
	  ...
	  $this->breadcrumb->append_crumb("Agenda Dinas", '/');
      ...
      $this->load->view($_SESSION['site_theme'].'/bidang/agenda_dinas/bg_home');
      $this->load->view($_SESSION['site_theme'].'/bg_right');
      $this->load->view($_SESSION['site_theme'].'/bg_footer');
   }
 
   public function detail($id_agenda,$tipe)
   {
      $where['id_multi_agenda'] = $id_agenda;
      $get_data = $this->db->get_where("dlmbg_multi_agenda",$where);
      $ret_data = $get_data->row();
      if($get_data->num_rows()>0)
      {
			$this->breadcrumb->append_crumb('Beranda', base_url());
			$this->breadcrumb->append_crumb('Indexs Agenda', base_url().'web/agenda');
		  ...
		  $d['dt_agenda_pengawas'] = $this->app_global_model->generate_menu_agenda_pengawas($_SESSION['limit_footer_artikel_sekolah'],0);
			...
			$d['dt_detail_agenda'] = $this->app_global_model->generate_detail_agenda($id_agenda,$tipe);

			$this->load->view($_SESSION['site_theme'].'/bg_header',$d);
			if($_SESSION['site_slider_always']=="yes"){$this->load->view($_SESSION['site_theme'].'/bg_slider');}
			$this->load->view($_SESSION['site_theme'].'/bg_breadcumb');
			$this->load->view($_SESSION['site_theme'].'/bg_left');
      $this->load->view($_SESSION['site_theme'].'/bidang/agenda_dinas/bg_detail');
			$this->load->view($_SESSION['site_theme'].'/bg_right');
			$this->load->view($_SESSION['site_theme'].'/bg_footer');
      }
      else
