...
class agenda extends MX_Controller {
   ...
   public function index($uri=0)
   {
      ...
      $this->breadcrumb->append_crumb('Indexs Agenda', '/');
      ...
      $d['dt_index_agenda'] = $this->app_global_model->generate_index_agenda($this->config->item('limit_item_medium'),$uri,$filter);
      ...
      $this->load->view($_SESSION['site_theme'].'/agenda/bg_home');
   ...
   public function detail($id_agenda,$tipe)
   {
      ...
      if($get_data->num_rows()>0)
      {
		  ...
		  $d['dt_agenda_pengawas'] = $this->app_global_model->generate_menu_agenda_pengawas($_SESSION['limit_footer_artikel_sekolah'],0);
			...
			$this->load->view($_SESSION['site_theme'].'/bg_breadcumb');
			$this->load->view($_SESSION['site_theme'].'/bg_left');
			$this->load->view($_SESSION['site_theme'].'/agenda/bg_detail');
			$this->load->view($_SESSION['site_theme'].'/bg_right');
			$this->load->view($_SESSION['site_theme'].'/bg_footer');
      }
      else
