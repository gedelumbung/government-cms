...
class agenda_pengawas extends MX_Controller {
   ...
   public function index($uri=0)
   {
		if($this->session->userdata("logged_in")!="" && $this->session->userdata("tipe_user")=="superadmin")
		{
			...
			$this->breadcrumb->append_crumb("Agenda Pengawas", '/');
			...
			$d['data_retrieve'] = $this->app_global_superadmin_model->generate_index_agenda_pengawas($this->config->item("limit_item"),$uri,$filter);
			
			$this->load->view('bg_header',$d);
			$this->load->view('agenda_pengawas/bg_home');
	...
	public function set()
	{
		if($this->session->userdata("logged_in")!="" && $this->session->userdata("tipe_user")=="superadmin")
		{
			...
			redirect("superadmin/agenda_pengawas");
		}
		else
		{
			redirect("web");
	...
	public function approve($id_param,$value)
	{
		if($this->session->userdata("logged_in")!="" && $this->session->userdata("tipe_user")=="superadmin")
		{
			$id['id_pengawas_agenda'] = $id_param;
			$up['stts'] = $value;
			$this->db->update("dlmbg_pengawas_agenda",$up,$id);
			redirect("superadmin/agenda_pengawas");
		}
		else
		{
